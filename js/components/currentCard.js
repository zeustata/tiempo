...
export function renderCurrentWeather(data, concejo, units = 'metric', iconTheme = 'astur') {
  ...
  const weatherInfo = getWeatherInfo(current.weather_code, current.is_day, current.precipitation, currentPop, directIrr);
    ...
    <div class="hero-weather-card ${weatherInfo.bg}">
      ...
      <div class="hero-main-row">
        ...
        <div class="temp-info-block">
          <div class="condition-name" title="${weatherInfo.isSolarCalibrated ? '☀️ Calibración Solar Inteligente: Radiación directa en tierra (> 100 W/m²) confirma sol y claros' : ''}">${weatherInfo.label}</div>
